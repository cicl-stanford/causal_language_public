---
title: "clang_forced_choice2_analysis"
...
c(num_excluded_participants = length(excluded_participants))
```
...
df_agg = right_join(df_agg, expand(df_agg, trial, response)) %>%
  group_by(trial) %>%
...
model_errors_with_movement = model_errors %>%
...
#           family = "cumulative",
#           file = "regression/ord_reg_flip_stationary",
#           seed = 1)
```
...
```{r Compare-the-regression-predictions-to-the-data}
reg = readRDS("regression/ord_reg_no_stationary_noise_1.6.rds")

set.seed(1)

mat_reg_pred = predict(reg, newdata = df_aspects)

df_reg_pred = as.data.frame(predict(reg, newdata = df_aspects)) %>% 
  rename(no_difference = "P(Y = 1)",
         affect = "P(Y = 2)",
         enable = "P(Y = 3)",
         cause = "P(Y = 4)") %>% 
...
reg_with_movement = readRDS("regression/ord_reg_noise_1.6.rds")

set.seed(1)

mat_reg_pred_with_movement =
  predict(
    reg_with_movement,
    newdata = df_aspects %>% mutate(stationary = 1 - moving)
  )

df_reg_pred_with_movement =
  as.data.frame(
    predict(
      reg_with_movement,
...
best_model_labels_with_movement = model_errors_with_movement %>%
  group_by(lesion_rsa) %>%
  summarise(model_label = model_label[[1]]) %>%
  ungroup() %>%
  pull(model_label)
best_models_with_movement = model_v_data_wide %>%
  filter(model_label %in% best_model_labels_with_movement) %>%
  mutate(model_label_short = if_else(lesion_rsa == "1", "No Pragmatics", "Full Model"))
```

```{r}
best_model_labels_with_movement
```

```{r}
best_models_all_with_movement = best_models_with_movement %>%
  select(trial, response, data_y, model_y, model_label_short) %>%
  rbind(
    df_reg_pred_with_movement %>%
      mutate(model_label_short = "Ordinal Regression") %>%
