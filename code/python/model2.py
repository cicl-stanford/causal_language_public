...
def aspect_rep(trial, candidate, target, alternatives, noise=2, perturb=0.01, num_samples=100):
	...
	if candidate in trial_entities:
		o = m.outcome(trial)
		w = m.whether_test(trial, candidate, target, noise, num_samples)
		h = float(m.how_test(trial, candidate, target, perturb, 1))
		s = m.sufficient_test(trial, candidate, target, alternatives, noise, num_samples)
		mov = m.moving_test(trial, candidate, target)
		return [w,h,s,mov,o]
	else:
		return [np.nan]*5
